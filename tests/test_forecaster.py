...
class TestDataPreparation:
    ...
    def test_prepare_monthly_data_returns_monthly(self, forecaster):
        ...
        # Check dates are month-start timestamps
    ...
    def test_prepare_monthly_data_for_specific_site(self, forecaster):
        ...
        # Different sites should have different volumes
    ...
    def test_fill_gaps_creates_continuous_range(self, forecaster):
        ...
        # All consecutive month gaps should be ~30-31 days
    ...
    def test_no_fill_gaps_preserves_original(self, forecaster):
        ...
        # Raw should have same or fewer rows (no interpolated months)
...
class TestOutlierHandling:
    ...
    def test_extreme_spike_is_capped(self, forecaster):
        ...
        # Inject a huge spike (100x normal)
...
class TestForecastGeneration:
    ...
    def test_ensemble_is_median_of_models(self, forecaster):
        """ENSEMBLE should be the median of individual model forecasts."""
        forecast = forecaster.generate_forecast(
            target_month="2025-03", site_id="100"
        )
        individual = forecast[~forecast["model"].isin(["ENSEMBLE", "FALLBACK"])]
        ensemble = forecast[forecast["model"] == "ENSEMBLE"]

        expected_median = individual["forecast_volume"].median()
        actual_ensemble = ensemble["forecast_volume"].iloc[0]
        assert actual_ensemble == pytest.approx(expected_median)
...
class TestBulkForecasts:
    ...
    def test_bulk_by_site(self, forecaster):
        """Bulk by-site should produce forecasts for multiple sites."""
        ...
        # Should have forecasts for both test sites
        sites = result["site_id"].unique()
        assert len(sites) >= 1

    def test_bulk_by_grade(self, forecaster):
        """Bulk by-grade should produce forecasts for each fuel grade."""
        result = forecaster.generate_bulk_forecasts(
            target_month="2025-03",
            by="grade",
            show_yoy=False,
        ...
        assert len(grades) >= 2  # UNL and DSL
    ...
    def test_bulk_by_site_grade(self, forecaster):
        ...
        assert not result.empty
        # Each site should have multiple grade entries
...
class TestSummarySheets:
    ...
    def test_site_summary_one_row_per_site(self, forecaster):
        ...
        if summary.empty:
            pytest.skip("No grade-level detail in test data")

        # Should NOT have a 'model' column (we filter to ENSEMBLE)
    ...
    def test_site_summary_matches_ensemble_sum(self, forecaster):
        ...
        # Pick a site and verify the sum
    ...
    def test_product_summary_groups_by_grade(self, forecaster):
        ...
        if summary.empty:
            pytest.skip("No data for product summary")

        # Each grade appears once
...
class TestExport:
    ...
    def test_export_to_excel(self, forecaster, tmp_path):
        ...
        # Verify file was created and has expected sheets
